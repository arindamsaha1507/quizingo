...
- A missed question passes to the opposing team for {{scoring.pass.reward}} points, with every remaining clue revealed first.
- Completing a qualifying line or region earns a {{scoring.bingo.reward}}-point Bingo bonus.
...
- Solving after the first, second, or third clue earns **{{scoring.wordle.clueRewards|words}} points**, respectively.
...
- The first revealed clue makes **{{scoring.ladders.clueRewards.0}} points** available.
- Revealing the second clue reduces the available score to **{{scoring.ladders.clueRewards.1}} points**.
- Revealing the third clue reduces it to **{{scoring.ladders.clueRewards.2}} points**.
...
- Solving after the first, second, third, or fourth clue earns **{{scoring.stringTheory.clueRewards|words}} points**, respectively.
...
- Only the first team to answer correctly earns **{{scoring.forbidden.reward}} points** and claims the centre cell.
...
6. If the active team does not answer correctly, the quizmaster reveals every remaining clue and passes the question to the opposing team for a flat **{{scoring.pass.reward}} points**.
...
For example, Team A opens a question on its turn and answers incorrectly. The question passes to Team B for {{scoring.pass.reward}} points. After the question is resolved, Team B still takes the next regular turn.
...
| Event | Points |
|---|---:|
| Wordle, after clues 1 / 2 / 3 | {{scoring.wordle.clueRewards|slashes}} |
| Ladders, after clues 1 / 2 / 3 | {{scoring.ladders.clueRewards|slashes}} |
| String Theory, after clues 1 / 2 / 3 / 4 | {{scoring.stringTheory.clueRewards|slashes}} |
| Correct pass answer | {{scoring.pass.reward}} |
| Forbidden | {{scoring.forbidden.reward}} |
| Each distinct Bingo | {{scoring.bingo.reward}} |
...
Teams earn Bingo bonuses by claiming complete patterns on the 5-by-5 grid. A Bingo adds **{{scoring.bingo.reward}} points** and does not end the game.
...
- If the active team does not answer correctly, reveal all remaining clues and offer the opposing team the {{scoring.pass.reward}}-point pass.
...
Team A chooses Question 7, a String Theory question. The quizmaster reveals two clues, and Team A submits the correct connection. Team A receives **{{scoring.stringTheory.clueRewards.1}} points** and claims cell 7.

If Team A had answered incorrectly, Team B could have attempted the same question for **{{scoring.pass.reward}} points**. A correct pass would give Team B the points and cell 7. Team B would still take the next regular turn because the pass does not alter the alternating turn order.
